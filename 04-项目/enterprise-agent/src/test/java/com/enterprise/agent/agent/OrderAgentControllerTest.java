...
class OrderAgentControllerTest {
    ...
    void orderReturnsReply() throws Exception {
        when(orderAgentService.chat(anyString())).thenReturn("订单 O1001：用户 张三，商品 机械键盘，金额 399.0 元，状态 PAID");
        ...
        mockMvc.perform(post("/agent/order")
                ...
                .andExpect(jsonPath("$.reply").value("订单 O1001：用户 张三，商品 机械键盘，金额 399.0 元，状态 PAID"));
    }
