...
/**
 * 企业订单 Agent 的工具集。
 * 每个 @Tool 方法 = Agent 可以调用的一项能力，模型根据用户意图自动选择。
 */
@Component
public class OrderTools {
    ...
    public String getOrder(@P("订单号，例如 O1001") String orderId) {
        return data.findOrderById(orderId)
                .map(order -> {
                    MockOrderData.User user = data.findUserById(order.userId()).orElse(null);
                    MockOrderData.Product product = data.findProductById(order.productId()).orElse(null);
                    return "订单 " + order.id()
                            + "：用户 " + (user != null ? user.name() : order.userId())
                            + "，商品 " + (product != null ? product.name() : order.productId())
