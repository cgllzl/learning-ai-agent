...
平均延迟像“全班平均分”，少数特别慢的请求很容易被平均掉。P95 更关注慢请求：

> 把 100 次请求按耗时从小到大排列，第 95 个请求的耗时就是 P95。换句话说，大约 95% 的请求不会比它更慢，最慢的约 5% 可能超过它。

本项目采用 nearest-rank（最近秩）算法。将延迟从小到大排序，位置为：
...
知道“现在有多慢”以后，下一个问题自然是“多慢才算合格”。这就是 SLO（Service Level Objective，服务水平目标）。

- SLI（Service Level Indicator）是实际测到的成绩，例如“本周 P95 为 1511 ms”；
- SLO 是团队事先规定的及格线，例如“本周 P95 不超过 2000 ms”；
- SLA（Service Level Agreement）是对外合同承诺，没达到时可能产生赔偿等后果。

可以把它们理解为：SLI 是考试分数，SLO 是学校的及格线，SLA 是写进合同的承诺。

以 Day 4 数据举例，如果**假设**生产目标是 `P95 <= 2000 ms`，那么 Single-Agent 的 1511 ms 达标，Multi-Agent 的 2488 ms 不达标。这里的 2000 ms 只是帮助理解的示例阈值，并不是当前项目已经配置的 SLO；当前代码也还没有实现 SLO 门禁。

而且，本日每个方案只有 5 个样本，此时 P95 实际等于最大值，只适合演示算法。生产 SLO 应基于更多样本、明确时间窗口和真实业务要求制定。
...
SLO 只能回答“方案是否达到最低要求”。如果多个方案都合格，我们还要判断谁值得保留，这才自然引出 Pareto（帕累托）比较。

这里说的是 **Pareto 支配**，不是常说的“二八定律”。可以把它想成两名学生比成绩：

> 我没有任何一科比你低，并且至少一科比你高，那么我就在这些科目上支配你。

对应到 Agent，方案 A 只有在以下指标都不差，并且至少一项严格更好时，才支配方案 B：
...
代码没有把这些指标乘成一个总分，因为“1% 成功率值多少钱”“多等 500 ms 能否接受”不是技术公式可以替业务决定的。加权总分还可能把某项明显退步藏在平均结果里。

Day 4 的真实结果可以逐项判断：

| 指标 | Single-Agent 相对 Multi-Agent | 是否更差 |
| --- | --- | --- |
| 成功率 | 相同，都是 100% | 否 |
| 平均成本 | 更低 | 否 |
| P95 延迟 | 更低 | 否 |
...
因此 Single-Agent 没有任何一项更差，并且三项更好，所以它在本次样本中 Pareto 支配 Multi-Agent。

如果另一项任务中 Multi-Agent 成功率更高，但同时更贵、更慢，那么双方各有长短，谁也不能支配谁。代码会返回 `REVIEW_TRADE_OFF`，让业务方决定是否值得为质量付出更多成本和等待时间。这些没有被其他候选全面击败的方案，合起来叫作 **Pareto 前沿**。
...
需要区分“推荐流程”和“当前实现”：当前项目已经实现三层评估与 Pareto 比较，但还没有把 SLO 阈值写成自动门禁。
